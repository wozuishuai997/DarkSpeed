...
/// NO once an anomaly may have left a thread inside the target process in a state we
/// no longer own. The bridge treats NO as a hard stop: no further remote calls are
/// issued, so a damaged thread cannot be resumed into a marker address and take
/// SpringBoard down with it.
